                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '3fr 2fr', gap: '40px' }}>
                        ...
                        <div>
                            {formData.skills && formData.skills.length > 0 && formData.skills.some(s => s.name) && (
                                <section style={{ /* pageBreakInside: 'avoid', */ marginBottom: '30px' }}>
                                    <h2 style={sectionTitleStyle}><FaTools /> Skills</h2>
                                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
                                        {formData.skills.map(skill => skill.name && (
                                            <span key={skill.id} style={{ backgroundColor: '#EDF2F7', color: '#4A5568', fontWeight: 600, padding: '5px 12px', borderRadius: '6px' }}>
                                                {skill.name}
                                            </span>
                                        ))}
                                    </div>
