     ...
     <div style={{display:'flex', width:'100%',height:'100%'}}>
      ...
      <aside style={leftColumnStyle}>
        <div style={{ textAlign: 'center', marginBottom: '30px' }}>
          ...
          <h2 style={{ fontSize: '16px', fontWeight: '400', color: '#A0AEC0' }}>{formData.title || "Professional Title"}</h2>
        </div>
        ...
        <div style={{ flexGrow: 1 }}>
          ...
          <ul style={{ listStyle: 'none', padding: 0, fontSize: '14px' }}>
            ...
            {formData.location && <li style={{ display: 'flex', alignItems: 'center', marginBottom: '10px' }}><FiMapPin style={{ marginRight: '10px' }}/> {formData.location}</li>}
              ...
              <div style={{pageBreakInside: 'avoid'}}>
                ...
                {formData.education.map(edu => (
                  <div key={edu.id} style={{ marginBottom: '15px' }}>
                    ...
                    <p style={{ margin: 0, color: '#A0AEC0', fontSize: '12px' }}>{edu.graduationDate}</p>
                  </div>
                ))}
              </div>
          )}
            ...
            {formData.experience.map(exp => (
              <div key={exp.id} style={{ marginBottom: '20px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <span style={{ fontSize: '12px', color: '#718096' }}>{exp.startDate} - {exp.endDate}</span>
                </div>
                <p style={{ margin: '5px 0', fontStyle: 'italic', color: '#4A5568' }}>{exp.company} | {exp.location}</p>
            ...
            <section style={sectionStyle}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
                    {formData.skills.map((skill, index) => (
                        <span key={index} style={{ background: '#EDF2F7', padding: '5px 12px', borderRadius: '15px', fontSize: '13px' }}>
                            {skill}
                        </span>
                    ))}
                </div>
